...
namespace Frankenstein
{
    public class Math3d
    {
        ...
        public static Vector2 Rotate2D(Vector2 input, float angle)
        {
        ...
        /// <summary>
        /// Rotates by using following method
        /// newX = centerX + ( cosX * (point2X-centerX) + sinX * (point2Y -centerY))
        /// newY = centerY + ( -sinX * (point2X-centerX) + cosX * (point2Y -centerY))
        /// </summary>
        /// <param name="input">Point to rotate</param>
        /// <param name="aroundPoint">Rotate around Point</param>
        /// <param name="angle">Degrees</param>
        /// <returns>Rotated Point, new Instance</returns>
        public static Vector2 Rotate2D(Vector2 input, Vector2 aroundPoint, float angle)
        {
            var p = new Vector2();

            p.x = aroundPoint.x + (input.x - aroundPoint.x) * Mathf.Cos(angle * Mathf.Deg2Rad) - (input.y - aroundPoint.y) * Mathf.Sin(angle * Mathf.Deg2Rad);
            
            p.y = aroundPoint.y + (input.x - aroundPoint.x) * Mathf.Sin(angle * Mathf.Deg2Rad) + (input.y - aroundPoint.y) * Mathf.Cos(angle * Mathf.Deg2Rad);
        ...
        //This function returns a point which is a projection from a point to a plane.
        public static List<Vector3> ProjectPointsOnPlane(Vector3 planeNormal, Vector3 planePoint, IList<Vector3> points)
        {
            var result = new List<Vector3>();
            for (int c = 0; c < points.Count; c++)
            {
                var p = ProjectPointOnPlane(planeNormal, planePoint, points[c]);
                result.Add(p);
            }
            return result;
        }
        ...
        /// <summary>
        /// Calculates the dot product as an angle
        /// </summary>
        ...
        /// <summary>
        /// Gets angle between two Vectors, range is 0 - 360
        /// NOT TESTED
        /// </summary>
        /// <param name="to"></param>
        /// <param name="from"></param>
        /// <param name="toFromUpVector">OPtional Axis of both, default is Vector3.up</param>
        /// <returns></returns>
        public static float Angle360(Vector3 to, Vector3 from, Vector3? toFromUpVector = null)
        {
            if(!toFromUpVector.HasValue)
                toFromUpVector = Vector3.up;
            
            return Quaternion.FromToRotation(toFromUpVector.Value, to - from).eulerAngles.z;
        }
        ...
        public static bool TriangleRayIntersect(Vector3 p1, Vector3 p2, Vector3 p3, Ray ray)
        {
        ...
        public static Vector3 GetCenter(IList<Vector3> verts)
        {
            ...
            return result / verts.Count;
        }

        public static Vector3 LocalToBarycentric(Vector3 p, Vector3 a, Vector3 b, Vector3 c)
        {
        ...
        public static Vector3 LocalToBarycentric2(Vector2 p, Vector2 v1, Vector2 v2, Vector2 v3)
        {
        ...
        public static Vector3 BarycentricToLocal(Vector3 bary, Vector3 a, Vector3 b, Vector3 c)
        {
            return (a * bary.x + b * bary.y + c * bary.z);
        }

        public static Matrix4x4 TriangleToTransformMatrix(Vector3 v1, Vector3 v2, Vector3 v3)
        {
            var a = v2 - v1;
            var b = v3 - v1;

            var c = Vector3.Cross(a, b);
            var magC = c.magnitude;
            if(magC>1)
                c = c.normalized;

            var b2 = Vector3.Cross(c, a).normalized;
            var a2 = a.normalized;
            var m = Matrix4x4.identity;
            m.SetColumn(0, new Vector4(a2.x, a2.y, a2.z, 0));
            m.SetColumn(1, new Vector4(b2.x, b2.y, b2.z, 0));
            m.SetColumn(2, new Vector4(c.x, c.y, c.z, 0));
            m = m.transpose;

            return m;
        }

        public static float ClaculateAreaHeron(Vector3[] vertices)
        {
            var s = 0f;
            var a = (vertices[0] - vertices[1]).magnitude;
            var b = (vertices[1] - vertices[2]).magnitude;
            var c = (vertices[2] - vertices[0]).magnitude;
        ...
        /// <summary>
        /// Calculates a tagent by given normal and related unit vectors.
        /// Two perpendicular vectors are needed to find tangent
        /// </summary>
        /// <param name="normal">The normal</param>
        /// <param name="lhs">Either unit vector or some prependicular vectors</param>
        /// <param name="rhs">Either unit vector or some prependicular vectors</param>
        /// <see cref="Frankenstein.API.Tests.Utils.Math3dTests.NormalToTagent"/>
        /// <returns>Tangent vector</returns>
        public static Vector3 NormalToTagent(Vector3 normal, Vector3 lhs, Vector3 rhs)
        {
            Vector3 tangent;
            Vector3 t1 = Vector3.Cross( normal, lhs );
            Vector3 t2 = Vector3.Cross( normal, rhs );
        ...
        /// <summary>
        /// Calculates UV Sphere coordinates by given point
        /// </summary>
        /// <param name="pos">Vector3 point</param>
        /// <returns>Vector2 UV</returns>
        public static Vector2 CalculateUVSphereCoordinates(Vector3 pos)
        {
            float u = Mathf.Atan2(pos.x, pos.z) / (-2f * Mathf.PI);
            float v = Mathf.Asin(pos.y) / Mathf.PI + 0.5f;
        ...
        public static class CircleMathHelper
        {
            public static float RadiusToSideLength(float r)
            {
                return r * (float)Math.Sqrt(3);
            }
            ...
            /// <returns></returns>
            public static Vector3[] CircleToTriangle(Vector3 center, float radius, int divideCount = 3)
            {
                var result = new Vector3[divideCount];

                for (int i = 0; i < divideCount; i++)
                {
                    var angle = i * (360 / divideCount);

                    var point = new Vector3();

                    point.x = center.x + radius * Mathf.Cos(Mathf.Deg2Rad * angle);
                    point.y = center.y + radius * Mathf.Sin(Mathf.Deg2Rad * angle);
